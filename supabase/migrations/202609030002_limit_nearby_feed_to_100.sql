-- Discovery is intentionally bounded: every filter combination exposes at
-- most the closest 100 eligible people. This keeps cursor pagination finite
-- and aligns the server result set with the client cache limit.
...
comment on function private.get_nearby_people_impl(text, integer, integer, uuid, integer)
  is 'Returns no more than the closest 100 eligible people for one filter set.';
